...
echo "<$BUILD_CDN/$FILE64|Windows x64 build $HASH (v$VERSION)> is ready for download." >> ../build_meta/notification.txt
# x32
echo "[Windows x32 build $VERSION ($HASH)]($BUILD_CDN/$FILE32) is ready for download." >> ../build_meta/comment.md
echo "<$BUILD_CDN/$FILE32|Windows x32 build $HASH (v$VERSION)> is ready for download." >> ../build_meta/notification.txt
